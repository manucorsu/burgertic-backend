...
const getPlatoById = async (id) => {
  ...
  try {
    const { rows } = await client.query("SELECT * FROM platos WHERE id = $1", [id]);
...
const deletePlato = async (id) => {
  ...
  try {
    const { rows } = await client.query("DELETE FROM platos WHERE id = $1", [id]);
...
const getPlatosByTipo = async (tipo) => {
  ...
  try {
    const { rows } = await client.query("SELECT * FROM platos WHERE tipo = $1", [tipo]);
