...
const getPedidosByUser = async (idUsuario) => {
  ...
  try {
    const { rows } = await client.query("SELECT * FROM pedidos WHERE id_usuario = $1", [idUsuario]);
...
const createPedido = async (idUsuario, platos) => {
  ...
  try {
    ...
    for (let plato of platos) {
      ...
      if (rows.length < 1) {
        await client.query("DELETE FROM pedidos WHERE id = $1", [idPedido]);
        await client.query("DELETE FROM pedidos_platos WHERE id_pedido = $1", [idPedido]);
        throw new Error("Plato no encontrado");
      }

      await client.query("INSERT INTO pedidos_platos (id_pedido, id_plato, cantidad) VALUES ($1, $2, $3)", [
        idPedido,
        plato.id,
        plato.cantidad,
      ]);
