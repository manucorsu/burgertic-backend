...
export const verifyToken = async (req, res, next) => {
  ...
  if (!authorization) return res.status(401).json({ message: "No se envió un header de autorización." });
    ...
    try {
      ...
      res.status(401).json({ message: error.message }); //este catch ya devuelve 'jwt expired' si el token venció, por lo que no escribimos código que verifique si el token está vencido o no
    }
};
...
export const verifyAdmin = async (req, res, next) => {
  ...
  try {
    ...
    if (!payload.id) {
      ...
      if (usuario === null) return res.status(404).json({ message: `No existe un usuario con el id ${payload.id}.` }); //No sé si es posible llegar a este caso si el login anda bien pero bue
